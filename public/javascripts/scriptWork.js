...
async function fetchTasks() {
    showLoader();
    const response=await fetch('/task/getTask?id=work');
    hideLoader();
    if(response.ok){
        console.log("okk");
    ...
    if(inputTask.value === ''){
        ...
        if(edit_id === null){
            ...
            const response=await fetch('/task/addTask',{
                method:"POST",
                body: JSON.stringify({newTask : newTask,today: false}),
                headers:{
            ...
            const response=await fetch('/task/editTask',{
                method:"PUT",
                body: JSON.stringify({prevTask : prevTask,newTask : newTask,today: false}),
                headers:{
                    'Content-type': 'application/json',
                }
            });
...
async function checkTask(idx) {
    ...
    const response=await fetch("/task/check",{
        method: "PATCH",
        body: JSON.stringify({task: taskArray[x],checked: checked,today: false}),
        headers:{
            'Content-type': 'application/json',
        }
    });
...
async function removeTask(idx){
    ...
    const response=await fetch("/task/delete",{
        method: "DELETE",
        body: JSON.stringify({task: taskArray[idx],today: false}),
        headers:{
            'Content-type': 'application/json',
        }
    });
...
async function genAI() {
    if(taskArray.length>0){
        ...
        document.getElementById("aiResponse").innerHTML=`<p>Add taask to generate response.</p>`;
    }
}